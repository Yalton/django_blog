...
//Function that returns true if the list is empty, and false if it has ItemNode.
...
{
	if (DEBUG)
		std ::cout << "[Player]DEBUG: Inserting a/an " << itemname(type, val) << " into ItemNode." << '\n';
	...
	if (type == 'S' || type == 'D' || type == 'O' || type == 'H')
	{
		...
		{
			if (DEBUG)
				std ::cout << "[Player]DEBUG: Adding Item to the front of the list" << endl;
			Player ::m_head = new Player ::ItemNode(x, m_head);
			return;
		}
		else
		{
			...
			{
				if (DEBUG)
					std ::cout << "[Player]DEBUG: Adding Item to the back of the list" << endl;
				tmp->m_next = new Player ::ItemNode(x, tmp->m_next);
				return;
			}
			else
			{
...
/* Removes specified Item from the list, can remove from anywhere in the list. */
bool Player ::remove(char c, int value)
{
	...
	{
		if (DEBUG)
			std ::cout << "[Player]DEBUG:Removing first Item in ItemNode list" << '\n';
		m_head = m_head->m_next;
		delete cur;
		return 1;
	}
	//If the Item is not first in the list; remove will search for it
	else if (m_head != NULL)
	{
		if(DEBUG)
		std ::cout << "[Player]DEBUG: Item not first, searching the rest of the list...." << '\n';
		...
		while (cur)
		{
			...
			if (cur->m_item.type == c)
			{
				if (cur->m_item.val == value)
				{
					...
					//If the Item is the last one in the list, then remove will delete it
					if (cur->m_next == NULL)
					{
						if (DEBUG)
					...
					//If Item is further than first or second, remove will do some patchwork to delete it from the list, and maintain continiuty.
					else
					{
						if (DEBUG)
...
/* Returns size of whole list, does not care about Item type or value */
int Player ::size()
{
	int x = 0;
	ItemNode *tmp;
...
/* Returns the number of ItemNode in the list with the specified type. */
...
/*Returns number of ItemNode in the list with specified type AND value. */
...
/* Returns true if an Item in the list exists with specified type AND value. */
